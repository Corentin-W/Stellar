{{-- resources/views/admin/support/templates/edit.blade.php --}}
@extends('layouts.astral-app')

@section('title', 'Modifier un Template - Admin Support')
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8">
        <div>
            <h1 class="text-3xl font-bold text-white mb-2">Modifier le Template</h1>
            <p class="text-gray-300">{{ $template->name }}</p>
        </div>
    ...
    <div class="bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-700 p-6">
        <form method="POST" action="{{ route('admin.support.templates.update', $template) }}" class="space-y-6">
            @csrf
            @method('PUT')
                ...
                <div class="bg-gray-700/50 rounded-lg p-4">
                    <div class="mb-4">
                        ...
                        <div id="preview-subject" class="text-white font-medium">
                            {{ old('subject', $template->subject) ?: 'Pas de sujet défini' }}
                        </div>
                    </div>
                    <div>
    ...
    <div class="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6">
        {{-- Activer/Désactiver --}}
        <div class="bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-700 p-6">
            <h3 class="text-lg font-medium text-white mb-2">Statut du Template</h3>
            <p class="text-gray-300 text-sm mb-4">
                {{ $template->is_active ? 'Ce template est actuellement actif et disponible pour utilisation.' : 'Ce template est désactivé et non disponible pour utilisation.' }}
            </p>
            <form method="POST" action="{{ route('admin.support.templates.toggle-status', $template) }}" class="inline">
                @csrf
                <button type="submit"
                        class="px-4 py-2 {{ $template->is_active ? 'bg-orange-600 hover:bg-orange-700' : 'bg-green-600 hover:bg-green-700' }} text-white rounded-lg transition duration-300">
                    {{ $template->is_active ? 'Désactiver' : 'Activer' }} le Template
                </button>
            </form>
        </div>
        ...
        <div class="bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-700 p-6">
            ...
            <form method="POST" action="{{ route('admin.support.templates.destroy', $template) }}" class="inline">
                ...
                <button type="submit"
                        ...
                        onclick="return confirm('Êtes-vous absolument sûr de vouloir supprimer ce template ? Cette action est irréversible.')">
    ...
    <div class="mt-6 bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-700 p-6">
        <h3 class="text-lg font-medium text-white mb-4">Informations du Template</h3>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 text-sm">
            <div>
                <div class="text-gray-400 mb-1">Créé par</div>
                <div class="text-white font-medium">{{ $template->creator->name }}</div>
            </div>
            <div>
                <div class="text-gray-400 mb-1">Date de création</div>
                <div class="text-white font-medium">{{ $template->created_at->format('d/m/Y à H:i') }}</div>
            </div>
            <div>
                <div class="text-gray-400 mb-1">Dernière modification</div>
                <div class="text-white font-medium">{{ $template->updated_at->format('d/m/Y à H:i') }}</div>
            </div>
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    variableButtons.innerHTML = '<div class="text-sm text-gray-400 mr-4">Insertion rapide :</div>';

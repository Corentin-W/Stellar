...
@section('title', 'Éditer le Template - ' . $template->name)
...
<div class="container mx-auto px-4 py-8">
    ...
    <form action="{{ route('admin.target-templates.update', $template) }}"
          method="POST"
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text"
                           name="template_id"
                           value="{{ old('template_id', $template->template_id) }}"
                ...
                <div>
                    ...
                    <input type="text"
                           name="constellation"
                           value="{{ old('constellation', $template->constellation) }}"
                ...
                <div>
                    ...
                    <select name="difficulty"
                            class="w-full px-4 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                            required>
                        <option value="beginner" {{ old('difficulty', $template->difficulty) == 'beginner' ? 'selected' : '' }}>Débutant</option>
                        <option value="intermediate" {{ old('difficulty', $template->difficulty) == 'intermediate' ? 'selected' : '' }}>Intermédiaire</option>
                        <option value="advanced" {{ old('difficulty', $template->difficulty) == 'advanced' ? 'selected' : '' }}>Avancé</option>
                    </select>
                    @error('difficulty')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text"
                           name="estimated_time"
                           value="{{ old('estimated_time', $template->estimated_time) }}"
                           placeholder="2h20min"
                           class="w-full px-4 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white">
            ...
            <div class="mt-6">
                ...
                <textarea name="short_description"
                          rows="2"
                          class="w-full px-4 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                          required>{{ old('short_description', $template->short_description) }}</textarea>
            ...
            <div class="mt-6">
                ...
                <textarea name="full_description"
                          rows="4"
                          class="w-full px-4 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white">{{ old('full_description', $template->full_description) }}</textarea>
            ...
            <div class="mt-6">
                ...
                <textarea name="tips"
                          rows="2"
                          class="w-full px-4 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white">{{ old('tips', $template->tips) }}</textarea>
                @error('tips')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
                ...
                <div>
                    ...
                    <div class="mb-3">
                        <img src="{{ asset('storage/' . $template->thumbnail_image) }}"
                             ...
                             class="h-32 w-auto rounded border">
                        <p class="mt-1 text-xs text-gray-500">Miniature actuelle</p>
                    </div>
                    @endif
                    <input type="file"
                ...
                <div>
                    ...
                    <div class="grid grid-cols-3 gap-2">
                        <div>
                            ...
                            <input type="number"
                                   name="ra_hours"
                                   value="{{ old('ra_hours', $template->ra_hours) }}"
                                   min="0" max="23"
                        ...
                        <div>
                            ...
                            <input type="number"
                                   name="ra_minutes"
                                   value="{{ old('ra_minutes', $template->ra_minutes) }}"
                                   min="0" max="59"
                                   class="w-full px-3 py-2 border rounded dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                                   required>
                        </div>
                        <div>
                            ...
                            <input type="number"
                                   name="ra_seconds"
                                   value="{{ old('ra_seconds', $template->ra_seconds) }}"
                ...
                <div>
                    ...
                    <div class="grid grid-cols-3 gap-2">
                        <div>
                            ...
                            <input type="number"
                                   name="dec_degrees"
                                   value="{{ old('dec_degrees', $template->dec_degrees) }}"
                                   min="-90" max="90"
                                   class="w-full px-3 py-2 border rounded dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                                   required>
                        </div>
                        <div>
                            ...
                            <input type="number"
                                   name="dec_minutes"
                                   value="{{ old('dec_minutes', $template->dec_minutes) }}"
                                   min="0" max="59"
                                   class="w-full px-3 py-2 border rounded dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                                   required>
                        </div>
                        <div>
                            ...
                            <input type="number"
                                   name="dec_seconds"
                                   value="{{ old('dec_seconds', $template->dec_seconds) }}"
                                   min="0" max="59.9" step="0.1"
                                   class="w-full px-3 py-2 border rounded dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                                   required>
                        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
                ...
                $selectedMonths = old('best_months', $template->best_months ?? []);
                @endphp
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number"
                           name="display_order"
                           value="{{ old('display_order', $template->display_order) }}"
                           class="w-full px-4 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white">

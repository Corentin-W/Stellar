...
@section('title', 'Modifier ' . $equipment->name . ' - Admin')
...
<div class="min-h-screen p-6" style="background: linear-gradient(135deg, #0a0a0f 0%, #1a1a2e 100%);">
    ...
    <div class="max-w-4xl mx-auto mb-8">
        <div class="flex items-center space-x-4">
            ...
            <div>
                <h1 class="text-3xl font-bold text-white mb-2">Modifier {{ $equipment->name }}</h1>
                <p class="text-white/60">Mettre à jour les informations de l'équipement</p>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-4xl mx-auto">
        <form action="{{ route('admin.equipment.update', $equipment) }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 p-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select id="type" name="type" required
                            ...
                            @foreach(\App\Models\Equipment::TYPES as $key => $label)
                                <option value="{{ $key }}" {{ old('type', $equipment->type) === $key ? 'selected' : '' }} class="bg-gray-800">
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select id="status" name="status" required
                            ...
                            @foreach(\App\Models\Equipment::STATUSES as $key => $label)
                                <option value="{{ $key }}" {{ old('status', $equipment->status) === $key ? 'selected' : '' }} class="bg-gray-800">
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" id="location" name="location" value="{{ old('location', $equipment->location) }}"
                    ...
                    <div>
                        ...
                        <input type="number" id="price_per_hour_credits" name="price_per_hour_credits"
                               value="{{ old('price_per_hour_credits', $equipment->price_per_hour_credits) }}"
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="description" name="description" rows="4"
                                  class="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-3 text-white placeholder-white/50 focus:outline-none focus:border-blue-500 focus:bg-white/20 transition-colors resize-vertical">{{ old('description', $equipment->description) }}</textarea>
                        @error('description')
                            <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    @if($equipment->specifications)
                        @foreach($equipment->specifications as $key => $value)
                            <div class="specification-row flex gap-4">
                                ...
                                <input type="text" name="spec_values[]" value="{{ is_array($value) ? json_encode($value) : $value }}" placeholder="Valeur"
            ...
            <div class="bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 p-8">
                        ...
                        <div class="relative group">
                            ...
                            <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity rounded-lg flex items-center justify-center">
                                <label class="cursor-pointer">
                                    ...
                                    <div class="w-8 h-8 bg-red-500/20 peer-checked:bg-red-500 rounded-lg flex items-center justify-center border-2 border-red-500 transition-colors">
                                        <svg class="w-4 h-4 text-red-400 peer-checked:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                                        </svg>
                                    </div>
                                </label>
                ...
                <p class="text-white/60 text-sm mt-4">Cochez les images à supprimer</p>
            </div>
            @endif
            ...
            <div class="bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 p-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    @foreach($equipment->videos as $video)
                        <div class="relative group">
                            <video class="w-full h-48 object-cover rounded-lg border border-white/20">
                                <source src="{{ Storage::url($video) }}" type="video/mp4">
                            </video>
                            <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity rounded-lg flex items-center justify-center">
                                <label class="cursor-pointer">
                                    <input type="checkbox" name="remove_videos[]" value="{{ $video }}" class="sr-only peer">
                                    <div class="w-8 h-8 bg-red-500/20 peer-checked:bg-red-500 rounded-lg flex items-center justify-center border-2 border-red-500 transition-colors">
                                        <svg class="w-4 h-4 text-red-400 peer-checked:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                                        </svg>
                ...
                <p class="text-white/60 text-sm mt-4">Cochez les vidéos à supprimer</p>
            </div>
            @endif
            ...
            <div class="bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 p-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <!-- Nouvelles images -->
                    <div>
                        <label for="new_images" class="block text-white/70 text-sm font-medium mb-2">
                            Nouvelles images (max 5MB chacune)
                        </label>
                    ...
                    <!-- Nouvelles vidéos -->
                    <div>
                        <label for="new_videos" class="block text-white/70 text-sm font-medium mb-2">
                            Nouvelles vidéos (max 50MB chacune)
                        </label>
                        <input type="file" id="new_videos" name="new_videos[]" multiple accept="video/*"
                    ...
                    <div class="flex items-center">
                        <label class="flex items-center cursor-pointer">
                            <input type="checkbox" name="is_featured" value="1"
                                   {{ old('is_featured', $equipment->is_featured) ? 'checked' : '' }}
                                   class="sr-only peer">
...
document.getElementById('new_videos').addEventListener('change', function(e) {
    const files = Array.from(e.target.files);

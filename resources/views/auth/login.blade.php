...
<div class="min-h-screen flex items-center justify-center px-6 py-12">
    <div class="max-w-md w-full">
        ...
        <div class="text-center mb-8">
            ...
            <p class="text-gray-400">{{ __('app.welcome') }} {{ __('app.app_name') }}</p>
        </div>
        ...
        <!-- Login Form -->
        <form method="POST" action="{{ route('login', app()->getLocale()) }}" class="space-y-6">
            @csrf
            ...
            <div class="flex items-center justify-between">
                <label class="flex items-center">
                    <input type="checkbox"
                           name="remember"
                           class="rounded border-gray-600 bg-transparent text-purple-500 focus:ring-purple-500 focus:ring-offset-0">
                    <span class="ml-2 text-sm text-gray-300">{{ __('auth.remember_me') }}</span>
                </label>
        ...
        <div class="text-center mt-6">
            <p class="text-gray-400">
                ...
                <a href="{{ route('register', app()->getLocale()) }}" class="text-purple-400 hover:text-purple-300 transition-colors font-medium">
                    {{ __('auth.register') }}
                </a>
            </p>
        </div>
...
<!-- Floating elements -->
<div class="fixed top-20 left-10 w-8 h-8 rounded-full bg-gradient-to-r from-blue-400 to-purple-500 opacity-20 animate-pulse"></div>
<div class="fixed bottom-20 right-10 w-6 h-6 rounded-full bg-gradient-to-r from-pink-400 to-yellow-400 opacity-30 animate-bounce" style="animation-delay: 1s;"></div>
<div class="fixed top-1/2 right-20 w-4 h-4 rounded-full bg-gradient-to-r from-green-400 to-blue-500 opacity-25 animate-ping" style="animation-delay: 2s;"></div>
@endsection
